---
version: alpha
name: Clinic-Shift-Scheduler-UI
description: "Framer-inspired dark-canvas system for the Clinic Shift Scheduler — pure black surfaces, white display type with aggressive negative tracking, white pill CTAs, charcoal cards, and scarce gradient spotlight tiles. Accent blue is reserved for links, focus, and selection only."
...
**This app ships Geist Sans (display) + Inter (body)** via `next/font` / `geist`. Token values live in `src/theme/tokens.ts` and CSS variables in `src/app/globals.css`. Ant Design is themed dark to match; primary CTAs are overridden to white pills (not accent-blue fills).
